...
type QuickFilter = 'jobs' | 'on_demand' | 'short_term' | 'full_time' | 'workers';
...
const quickFilters: Array<{ value: QuickFilter; label: string }> = [
  ...
  { value: 'workers', label: 'Labourers available' },
];
...
export default function ClassifiedsPage() {
  ...
  const filteredListings = useMemo(() => {
    ...
    return listings
      .filter((listing) => {
        if (quickFilter === 'workers') return listing.kind === 'worker';
  ...
  return (
    ...
    >
      <Seo
        title="Okanagan Trades Classifieds | Jobs and Labourers Available"
        description="Browse moderated Okanagan trades classifieds for hiring, short-term labour, full-time roles, and labourers available for work."
        path="/classifieds"
      />
      ...
      <div className="mx-auto max-w-[96rem] px-4 sm:px-6 lg:px-10">
        <div className="grid gap-8 border-b border-zinc-200 pb-10 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div>
            ...
            >
              Moderated classifieds
            </SectionEyebrow>
            <h1 className="mt-6 text-4xl font-bold uppercase leading-tight text-zinc-950 sm:text-5xl lg:text-6xl">
              Trades hiring and labour availability
            </h1>
            <p className="mt-4 max-w-3xl text-lg leading-8 text-zinc-600">
              Find approved local job posts, short-term work, on-demand help, and labourers available across the Okanagan.
            </p>
          </div>
          <Link
            to="/classifieds/post"
          ...
          <section className="mt-8">
            ...
            {filteredListings.length === 0 ? (
              <div className="border border-zinc-200 bg-white px-6 py-12 text-center shadow-sm">
                <p className="text-lg font-semibold text-zinc-950">No approved listings match these filters.</p>
                <p className="mt-2 text-sm text-zinc-500">Post a new listing and it will appear here after admin review.</p>
              </div>
            ) : (
